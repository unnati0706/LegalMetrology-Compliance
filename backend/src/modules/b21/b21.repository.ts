...
import { CheckResult, Declaration, Rule } from '../../shared/types/index.js';

export class B21Repository {
  public findRulesByCategory(category: string, version: string): Rule[] {
    return Array.from(db.store.rules.values()).filter(
      r => r.category === category && r.version === version && r.isActive
    );
  }
  ...
  public saveDeclarations(declarations: Omit<Declaration, 'id' | 'createdAt' | 'updatedAt'>[]): Declaration[] {
    ...
    for (const d of declarations) {
      ...
      db.store.declarations.set(entity.id, entity);
      saved.push(entity);
    }
    return saved;
  }
