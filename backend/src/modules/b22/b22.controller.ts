...
import { B22Service } from './b22.service.js';
import { ApiResponse } from '../../shared/types/index.js';

export class B22Controller {
  constructor(private service: B22Service = new B22Service()) {}
  ...
  public evaluate = async (req: Request, res: Response<ApiResponse>, next: NextFunction): Promise<void> => {
    try {
      ...
      const result = await this.service.evaluateMrpAndQuantity(req.body, userId);
      res.status(200).json({
        success: true,
        data: result,
        meta: {
