...
import { B24Service } from './b24.service.js';
import { ApiResponse } from '../../shared/types/index.js';

export class B24Controller {
  constructor(private service: B24Service = new B24Service()) {}
  ...
  public evaluate = async (req: Request, res: Response<ApiResponse>, next: NextFunction): Promise<void> => {
    try {
      ...
      const result = await this.service.evaluateDateAndPlacement(req.body, userId);
      res.status(200).json({
        success: true,
        data: result,
        meta: {
